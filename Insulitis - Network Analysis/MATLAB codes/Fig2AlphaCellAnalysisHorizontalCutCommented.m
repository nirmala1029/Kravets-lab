%% This code slices an islet into two halves using a horizontal line, and computes the percentage of islet cells and immune cells in each half
...
for isletno = 1:1 %Update this line with a list of islets to be analyzed
    ...
    % Split the bounding box into two halves using a horizontal line, and store each half as different masks

    bboxmask{1}(uppervert:uppervert+partheight,leftvert:rightvert) = 1;
    bboxmask{2}(uppervert+partheight+1:lowervert,leftvert:rightvert) = 1;
    ...
    Isletcellsold = table2array(readtable([filepath,['islet ', num2str(isletno), ' insulin position.xlsx']])); %Replace with either alpha or beta cells
    Immunecellsold = table2array(readtable([filepath,['islet ', num2str(isletno), ' f480 position.xlsx']])); %Replace with T-, macrophage or myeloid cells
    ...
    for j=1:2
        ...
        yMask = Selection{1}(:,1)*0.4964671; % Convert the x coordinates of the vertices into um
        xMask = Selection{1}(:,2)*0.4964671; % Convert the y coordinates of the vertices into um 
